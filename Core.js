...
case 'handsomecheck':
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
				if (!text) return replay(`Tag Someone, Example : ${prefix + command} @Dipanjan`)
...
case 'charactercheck':
 ...
 case 'dare':
    ...
    if (isBanChat) return reply(mess.bangc)
         ...
         "say i love the bot owner Dipanjan through voice note",
...
speak, miku, stalk, profile, help, ihelp(iphone), delete, deleteall, listgc, listpc, welcome, support, repo, script 
...
speak, miku, stalk, profile, help, ihelp(ihone), delete, deleteall, listgc, listpc, welcome, support, repo, script 
